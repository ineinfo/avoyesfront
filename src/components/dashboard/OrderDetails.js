...
import { fetchCities, fetchColors, fetchCountries, fetchSizes, fetchStates } from "@/utils/api/CommonApi";
...
const OrderDetails = ({ change }) => {
  ...
  useEffect(() => {
    ...
    if (Id) orderDetails(Id); allSizes(); allColors();
  }, [Id]);
  ...
  const { order_id, order_number, created_at, order_items, final_amount } = details;
  ...
  return (
    <>
      <div className="my-order-head">
        ...
        <div className="ordr-dtl-img text-center pt-5" style={{ display: "flex", justifyContent: "left", gap: "10px" }}>
          {order_items?.map((item) => (
            <div key={item.id} className="item" style={{ display: "flex", justifyContent: "center", gap: "5px" }}>
              <img src={item?.image_url || "/sidebar-img-1.png"} alt="" />
              <div className="dtl " style={{ textAlign: "left" }}>
                <h5 className="m-0">{item.item_name}</h5>
                <div style={{ display: "flex", gap: "5px" }}> <p className="m-0">Size : {getSize(item.size_id || 'Free Size')}</p>
                  <p className="m-0">color : {getColor(item.color_id || '')}</p></div>
        ...
        <div className="order-track-main">
          <div className="arriving d-flex align-items-center">
            ...
            <div className="text">
              ...
              <p className="m-0">Order Placed On {new Date(created_at).toLocaleDateString('en-GB', {
                day: 'numeric',
                month: 'long',
                year: 'numeric',
              })}</p>
        ...
        <div className="delivery-address-box">
          ...
          <div className="text-1 d-flex align-items-center contact-name-phone">
            <div className="name">
              <p className="m-0 pe-3"> {address ? `${address.first_name} ${address.last_name}` : ''}</p>
            </div>
            <div className="phone">
          ...
          <div className="add">
            <p className="m-0">{address ? `${address.address1} , ${getCity(address.city)} , ${getState(address.state)} , ${address.pincode} , ${countryname}` : ''}</p>
          </div>
        </div>
          ...
          <div className="text-1 d-flex align-items-center pt-2">
            <div className="name">
              <p className="m-0 pe-3">
                ...
                >
                  {" "}
                  <i className="fa-solid fa-envelope me-2"></i>{address ? address.email : ''}
                </Link>
              </p>
            </div>
            <div className="phone">
              <p className="m-0 ps-3">
                <Link href="tel:1234567890" className="text-decoration-none">
                  {" "}
                  <i className="fa-solid fa-phone me-2"></i> {address ? address.phone : ''}
                </Link>
              </p>
            </div>
          </div>
